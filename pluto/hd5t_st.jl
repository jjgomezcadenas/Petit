...
md"""
- Reads "single track files", generated by script batch\_track\_analysis_mt.jl
...
# ╔═╡ 6f69bce5-5818-41b0-86db-612ef06644d0
trkb214[i]

# ╔═╡ ce0c0477-0b99-400a-a85e-f8e6cad5e094
let
	xout =jn.Petit.find_track_extremes(trkb214, i=i)
	extremes, _, _, xtrack_length, confidence = xout
	xstart_voxel, xend_voxel = extremes
	blobs = jn.Petit.energy_in_spheres_around_extremes(trkb214[i], xout, rblob)
...
# ╔═╡ e4f072f1-e454-4504-b95f-b5867af00b60
rblob

# ╔═╡ 1c202304-d6ca-4dde-a7ac-f5cb2b961b93
begin
p2d, p3d = jn.Petit.plot_track_blobs(trkb214[i], rblob;
...
# ╔═╡ d98b7b10-4919-4db5-95df-ee73212100f1
p3d
...
let
	xout =jn.Petit.find_track_extremes(trktl208, i=i)
	extremes, _, _, xtrack_length, confidence = xout
	xstart_voxel, xend_voxel = extremes
	blobs = jn.Petit.energy_in_spheres_around_extremes(trktl208[i], xout, rblob)
	eb1 = blobs.blob1_energy * 1e+3
	eb2 = blobs.blob2_energy * 1e+3
	nb1 = blobs.blob1_voxel_count
	nb2 = blobs.blob2_voxel_count
...
begin
p2dTl208, p3dTl208 =jn.Petit.plot_track_blobs(trktl208[i], rblob;
                         markersize_voxels=3.0,
                         show_connections=true,
                         alpha_connections=0.2,
                         alpha_spheres=0.3,
...
# ╔═╡ 7c133508-0267-4217-bc03-0424c99e1136
p3dTl208
...
let
	xout =jn.Petit.find_track_extremes(trksbb0, i=i)
	extremes, _, _, xtrack_length, confidence = xout
	xstart_voxel, xend_voxel = extremes
	blobs = jn.Petit.energy_in_spheres_around_extremes(trksbb0[i], xout, rblob)
	eb1 = blobs.blob1_energy * 1e+3
	eb2 = blobs.blob2_energy * 1e+3
	nb1 = blobs.blob1_voxel_count
	nb2 = blobs.blob2_voxel_count
...
begin
	bb0nuP2d, bb0nuP3d = jn.Petit.plot_track_blobs(trksbb0[i], rblob;
                         markersize_voxels=3.0,
                         show_connections=true,
                         alpha_connections=0.2,
                         alpha_spheres=0.3,
...
# ╔═╡ aacea693-49fa-49ef-9d16-466d8639dd11
bb0nuP3d
...
let
	xout =jn.Petit.find_track_extremes(trksxe1e, i=i)
	extremes, _, _, xtrack_length, confidence = xout
	xstart_voxel, xend_voxel = extremes
	blobs = jn.Petit.energy_in_spheres_around_extremes(trksxe1e[i], xout, rblob)
	eb1 = blobs.blob1_energy * 1e+3
	eb2 = blobs.blob2_energy * 1e+3
	nb1 = blobs.blob1_voxel_count
	nb2 = blobs.blob2_voxel_count
...
begin
	xe137P2d, xe137P3d = jn.Petit.plot_track_blobs(trksxe1e[i], rblob;
                         markersize_voxels=3.0,
                         show_connections=true,
                         alpha_connections=0.2,
                         alpha_spheres=0.3,
...
# ╔═╡ 0423b02f-09f1-4f69-b76b-cfd7841c1932
xe137P3d
...
let
	xout =jn.Petit.find_track_extremes(trksxe1ev1, i=i)
	extremes, _, _, xtrack_length, confidence = xout
	xstart_voxel, xend_voxel = extremes
	blobs = jn.Petit.energy_in_spheres_around_extremes(trksxe1ev1[i], xout, rblob)
	eb1 = blobs.blob1_energy * 1e+3
	eb2 = blobs.blob2_energy * 1e+3
	nb1 = blobs.blob1_voxel_count
	nb2 = blobs.blob2_voxel_count
...
begin
	xe1371mmP2d, xe1371mmP3d =jn.Petit.plot_track_blobs(trksxe1ev1[i+1], rblob;
                         markersize_voxels=3.0,
                         show_connections=true,
                         alpha_connections=0.2,
                         alpha_spheres=0.3,
...
# ╔═╡ 144ca850-92b9-450b-8d8f-07015c92c1b3
xe1371mmP3d
...
# ╔═╡ 22244695-6483-4453-97ef-65ee6c58f6e1
function fom_plots(results::Vector{DataFrame}, 
				   labels::Vector{String},title::String="", ylims=(0.0, 1.0))
	p = scatter(
...
# ╔═╡ 7cc4d23a-e039-48e3-ae9e-baee8477f97f
let
	results = [effBlbBB, effBlbBBv1R5]
	labels = ["bb0nu", "bb0nu1mm"]
